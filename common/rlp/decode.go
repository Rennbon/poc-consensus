...
func makeDecoder(typ reflect.Type, tags tags) (dec decoder, err error) {
	...
	default:
		return nil, fmt.Errorf("func makeDecoder rlp: type %v is not RLP-serializable", typ)
	}
}
...
func decodeInterface(s *Stream, val reflect.Value) error {
	if val.Type().NumMethod() != 0 {
		return fmt.Errorf("func decodeInterface rlp: type %v is not RLP-serializable", val.Type())
	}
	kind, _, err := s.Kind()
	if err != nil {
		return err
