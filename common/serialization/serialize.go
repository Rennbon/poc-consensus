...
func ReadComplex(reader io.Reader) (*sink.ComplexType, error) {
	fb := [2]byte{}
	_, err := io.ReadFull(reader, fb[:2])
	if err != nil {
		return nil, err
	}
	size := common.BytesToUint16(fb[:])
	fb2 := [1]byte{}
	_, err = io.ReadFull(reader, fb2[:])
	if err != nil {
		return nil, err
	}
	fb3 := make([]byte, size)
	_, err = io.ReadFull(reader, fb3[:])
	if err != nil {
		return nil, err
	}
	cp := &sink.ComplexType{
...
/*func ReadRealase(r io.Reader, typ reflect.Type) (out interface{}, err error) {
	switch typ {
	...
	case common.BType_BigInt:
		cp := &sink.ComplexType{}
		cp, err = ReadComplex(r)
		if err != nil {
			return nil, err
		}
		out, _ = cp.ComplexToBigInt()
	case common.BType_Hash:
		var byteArr []byte
		byteArr, err = ReadBytes(r, common.HashLength)
		if err != nil {
			return nil, err
		}
		out, err = common.ParseHashFromBytes(byteArr)
	case common.BType_Address:
		var addr []byte
		addr, err = ReadBytes(r, common.AddrLength)
		if err != nil {
			return nil, err
		}
		out = common.Bytes2Address(addr)
	case common.BType_Symbol:
		var symArr []byte
		symArr, err = ReadBytes(r, common.SymbolLen)
		if err != nil {
			return nil, err
		}
		out = common.Bytes2Symbol(symArr)
